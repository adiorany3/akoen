...
def convert_ambil_to_byurule(input_files=['combined_proxies.yaml'], template_file='byurule.yaml', 
                           output_file='newbyurulecombine.yaml', telegram_bot_token=None, 
                           telegram_chat_id=None, telegram_enabled=True):
    """Convert multiple YAML input files to newbyurulecombine.yaml with enhanced proxy configurations"""
            ...
            new_data['proxy-groups'] = [
                {
                    'name': 'Selector',
                    'type': 'select',
                    'proxies': ['Fallback', 'URL-Test', 'Load-Balance', 'RG', 'Ilped', 'GGWP'] + proxy_names
                },
                {
                    'name': 'Fallback',
                    'type': 'fallback',
